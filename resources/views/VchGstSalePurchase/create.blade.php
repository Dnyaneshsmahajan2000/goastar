...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            ?>
            @if ($action == 'add')
                <form action="{{ route('vch.gst.store', $vch_type) }}" method="POST" id="fg-form"
                    class="needs-validation" novalidate>
                @else
                    <form action="{{ route('vch.gst.update', ['vch_type' => $vch_type, 'voucher' => 1]) }}" id="fg-form"
            ...
            <div class="row g-2">
                ...
                <div class="col-xxl-2 col-md-2">
                    ...
                    <input type="text" class="form-control form-control-sm" id="invoicenoInput" autofocus
                        placeholder="Invoice No" name="ref_no" value='{{ $id }}' />
                ...
                <div class="col-xxl-2 col-md-3">
                     <label class="">
                        Party<span style="margin-right: 10px;"></span>Balance: <span id="balance"></span> ||
                        Credit Limit: <span id="credit_data"></span>
                        <a href="{{ route('ledger.create') }}">
                            <span style="color: red; cursor: pointer;">( Add Ledger )</span>
                        </a>
                    </label>
                    <input type="hidden" class="form-control form-control-sm" name="ledger_id" id="ledger_id"
                        value="<?php echo isset($voucher['ledger_id']) ? $voucher['ledger_id'] : ''; ?>">
                    <input type="text" name="ledger_name" id="ledger_name" required autofocus class="form-control form-control-sm"
                        value="<?php echo isset($voucher->ledger->name) ? $voucher->ledger->name : ''; ?>">
                    ...
                    <select class="form-control form-select form-select-sm" autofocus required=""
                        ...
                        @if (Auth::user()->role->name == 'SuperAdmin')
                            @foreach ($godowns as $godown)
                                <option value="{{ $godown['id'] }}" @if (isset($voucher['godown_id']) && $voucher['godown_id'] == $godown['id']) selected @endif>
                                    {{ $godown['name'] }}
                                </option>
                            @endforeach
                        @else
 ...
 --}}
        ...
        <div class="card-footer">
            ...
            <div class="col-md-12">
                ...
                <div class="table-responsive mt-2" id='item_list'>
                    @include('VchGstSalePurchase.vch-items')
    ...
    var availableItems = [
        @foreach ($items as $data)
            @if ($vch_type == 'sale' || $vch_type == 'order')
                {
                    label: '{{ $data['label'] }} ( {{ $data['stock_qty'] }} piece )',
                    id: '{{ $data['id'] }}',
                    rate: '{{ $data['rate'] }}',
    ...
    $("#ledger_name").autocomplete({
        source: availableLedgers,
        select: function(event, ui) {
            $('#ledger_id').val(ui.item.id);
            $('#ledger_name').val(ui.item.label);
              $('#credit_limit').text(ui.item.credit_limit); // Update the credit_limit field
                    $('#balance').text(ui.item.balance); // Update the balance field
            ...
            $('#product_form').on('submit', function(event) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        $("#item_list").load('{{ route('vch.item.list', 'gst') }}');
                    },
