        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('report.groupreport') }}" method="GET" class="needs-validation" id="report_form"
                        novalidate>
                                    ...
                                    <select name="group_id" id="group_id" class="form-select">
                                        @foreach ($groups_data as $group)
                                            <option value="{{ $group->id }}"
                                                @if (isset($group_id) && $group_id == $group->id) selected @endif>{{ $group->group_name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
        ...
        <div class="col-lg-12">
            <div class="card p-2">
                <div class="card-header text-center bg-primary text-white">
                    <h4 class="text-white">Report Summary {{ isset($group_name) ? $group_name : '' }}</h4>
                            ...
                            <tbody>
                                ...
                                @if (isset($groups))
                                    @foreach ($groups as $group)
                                        <tr>
                                            <td>
                                                <a href="groupreport?group_id={{ $group['id'] }}&submit=">
                                                    {{ $group['group_name'] }}
                                                </a>
                                ...
                                @if (isset($ledgers))
                                    @foreach ($ledgers as $ledger)
                                        <?php
                                        $opening_balance = \App\Models\transaction::where('ledger_id', $ledger->id)
                                        ...
                                        $transactions = \App\Models\transaction::where('ledger_id', $ledger->id)
